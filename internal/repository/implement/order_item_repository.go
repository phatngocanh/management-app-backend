...
func (repo *OrderItemRepository) CreateCommand(ctx context.Context, item *entity.OrderItem, tx *sqlx.Tx) error {
	insertQuery := `INSERT INTO order_items(order_id, product_id, bom_id, quantity, selling_price, original_price, discount_percent, final_amount) 
					VALUES (:order_id, :product_id, :bom_id, :quantity, :selling_price, :original_price, :discount_percent, :final_amount)`
...
func (repo *OrderItemRepository) UpdateCommand(ctx context.Context, item *entity.OrderItem, tx *sqlx.Tx) error {
	updateQuery := `UPDATE order_items SET order_id = :order_id, product_id = :product_id, bom_id = :bom_id, 
