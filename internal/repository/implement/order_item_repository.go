...
func (repo *OrderItemRepository) CreateCommand(ctx context.Context, orderItem *entity.OrderItem, tx *sqlx.Tx) error {
	insertQuery := `INSERT INTO order_items(order_id, product_id, quantity, selling_price, cost, discount, final_amount, export_from) VALUES (:order_id, :product_id, :quantity, :selling_price, :cost, :discount, :final_amount, :export_from)`
