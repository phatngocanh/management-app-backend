...
func (s *OrderImageService) GenerateSignedUploadURL(ctx *gin.Context, orderID int, fileName string, contentType string) (model.GenerateOrderImageSignedUploadURLResponse, string) {
	...
	if err != nil {
		...
		return model.GenerateOrderImageSignedUploadURLResponse{}, error_utils.ErrorCode.INTERNAL_SERVER_ERROR
	}
	...
	if err != nil {
		...
		return model.GenerateOrderImageSignedUploadURLResponse{}, error_utils.ErrorCode.DB_DOWN
	}

	response := model.GenerateOrderImageSignedUploadURLResponse{
		SignedURL: signedURL,
		ImageKey:  s3Key,
